...
## partnerBrain更新の自律運用(全環境共通・強制ルール)

partnerBrainは Claude 側の前提知識ベース。**更新・追記・新規ファイル作成は同意を求めず Claude 判断で自走してよい**(=Kainaの整理領域ではなくClaudeの整理領域)。

- 会話で得た新しい前提情報(認知特性・関係性・業務状況・趣味の更新等)が出たら、適切な層(summary/details)に反映する
- 既存ファイルへの追記、新規ファイル作成、ファイル間リンクの整備、`partnerBrain/_index.md` 更新まで含めて自走する
- 複数案提示・方針相談・許可取りは不要(運用に支障が出てきたタイミングで Kaina から方針修正が入る前提)
- 例外: **ファイル削除や大規模な構造変更**(層の再編、ファイル統廃合)は事前に確認する
- 更新時はVault内整合性を保つ(関連ノートとリンク、index反映)
- summary層は肥大化させない(必読層なのでロードコスト直撃)。詳細はdetails層に分離する判断をデフォルトに
